...
switch $opt(version) {
    "v04" {
        solution options set /Input/CompilerFlags {-DV04}
    }
    "v05" {
        solution options set /Input/CompilerFlags {-DV05}
    }
    default {
        # defaults
    }
}
solution options set /Input/SearchPath {../inc ../src ../tb ../weights}

# Add source files.
solution file add ../src/myproject.cpp -type C++
...
switch $opt(version) {
    "v04" {
        solution design set {nnet::relu<ac_fixed<16, 6, true, AC_TRN, AC_WRAP>, ac_fixed<10, 0, false, AC_RND_CONV, AC_SAT>, relu_config4>} -inline
        solution design set ac::fx_div<8> -inline
        solution design set {dense<ac_fixed<16, 6, true, AC_TRN, AC_WRAP>, ac_fixed<16, 6, true, AC_TRN, AC_WRAP>, config2>} -inline
        solution design set {dense<ac_fixed<10, 0, false, AC_RND_CONV, AC_SAT>, ac_fixed<16, 6, true, AC_TRN, AC_WRAP>, config5>} -inline
    }
    "v05" {
#        solution design set myproject -top
...
if {$opt(hsynth)} {
    ...
    switch $opt(version) {
        "v04" {
            ...
            directive set /myproject/layer5_out:rsc -MAP_TO_MODULE ccs_ioport.ccs_out
            directive set /myproject/w2:rsc -MAP_TO_MODULE ccs_ioport.ccs_in
            directive set /myproject/w2:rsc -EXTERNAL_MEMORY true
            directive set /myproject/w2:rsc -PACKING_MODE sidebyside
            directive set /myproject/w2 -WORD_WIDTH 8960
            directive set /myproject/b2:rsc -MAP_TO_MODULE ccs_ioport.ccs_in
            directive set /myproject/b2:rsc -EXTERNAL_MEMORY true
            directive set /myproject/b2:rsc -PACKING_MODE sidebyside
            directive set /myproject/b2 -WORD_WIDTH 640
            directive set /myproject/w5:rsc -MAP_TO_MODULE ccs_ioport.ccs_in
            directive set /myproject/w5:rsc -EXTERNAL_MEMORY true
            directive set /myproject/w5:rsc -PACKING_MODE sidebyside
            directive set /myproject/w5 -WORD_WIDTH 1920
        }
        "v05" {
#            directive set /myproject/input_1:rsc -MAP_TO_MODULE ccs_ioport.ccs_in
...
#            directive set /myproject/b5:rsc -MAP_TO_MODULE ccs_ioport.ccs_in
    ...
    switch $opt(version) {
        "v04" {
            directive set /myproject/core/main -PIPELINE_INIT_INTERVAL 1
            directive set /myproject/core/ACCUM_OUTER_LOOP -UNROLL yes
            directive set /myproject/core/ACCUM_INNER_LOOP -UNROLL yes
            directive set /myproject/core/nnet::relu<layer2_t,layer4_t,relu_config4>:for -UNROLL yes
            directive set /myproject/core/ACCUM_OUTER_LOOP#1 -UNROLL yes
            directive set /myproject/core/ACCUM_INNER_LOOP#1 -UNROLL yes
        }
        "v05" {
